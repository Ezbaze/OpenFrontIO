...
(function () {
    ...
    function renderLandmassView(options) {
        const { leaf, snapshot, sortState, onSort, existingContainer, lifecycle } = options;
        ...
        const playerLookup = new Map(snapshot.players.map((player) => [player.id, player]));
        const landmasses = [...snapshot.landmasses].sort((a, b) => compareLandmasses({ a, b, sortState }));
        for (const landmass of landmasses) {
            ...
            for (const column of LANDMASS_HEADERS) {
                const td = createElement("td", cellClassForColumn(column, getLandmassExtraCellClass(column.key)));
                switch (column.key) {
                    case "label": {
                        ...
                        wrapper.appendChild(createPlayerNameElement(landmass.ownerName, focus, {
                            asBlock: true,
                            className: "block font-semibold text-slate-100 transition-colors hover:text-sky-200",
                        }));
                        wrapper.appendChild(createElement("span", "text-[0.65rem] uppercase tracking-wide text-slate-400", `Landmass #${landmass.sequence}`));
    ...
    function createTableShell(options) {
        ...
        for (const column of headers) {
            ...
            const button = createElement("span", `flex w-full items-center gap-1 text-inherit ${column.align === "left"
            ? "justify-start"
            : column.align === "right"
                ? "justify-end"
    ...
    function registerContextMenuDelegation(container, actions) {
        ...
        const handleContextMenu = (event) => {
            ...
            const tradingPlayers = target.players.filter((player) => !(player.tradeStopped ?? false));
            const stoppedPlayers = target.players.filter((player) => player.tradeStopped ?? false);
    ...
    function appendPlayerRows(options) {
        const { player, indent, leaf, snapshot, tbody, requestRender, metricsCache } = options;
        ...
        const firstCell = createElement("td", "border-b border-r border-slate-800 border-slate-900/80 px-3 py-2 align-top last:border-r-0");
        ...
        if (expanded) {
            ...
            const detailCell = createElement("td", "border-b border-slate-800 px-4 py-4");
    ...
    function appendGroupRows(options) {
        const { group, leaf, snapshot, tbody, requestRender, groupType, metricsCache, } = options;
    ...
    function renderPlayerDetails(player, snapshot) {
        ...
        grid.appendChild(createDetailSection("Incoming attacks", player.incomingAttacks, (attack) => `${attack.from} – ${formatNumber(attack.troops)} troops`));
        grid.appendChild(createDetailSection("Outgoing attacks", player.outgoingAttacks, (attack) => `${attack.target} – ${formatNumber(attack.troops)} troops`));
        grid.appendChild(createDetailSection("Defensive supports", player.defensiveSupports, (support) => `${support.ally} – ${formatNumber(support.troops)} troops`));
    ...
    function createLabelBlock(options) {
        const { label, subtitle, indent, expanded, toggleAttribute, rowKey, onToggle, focus, } = options;
  ...
  `;
    ...
    class SidebarApp {
        ...
        createSidebarShell() {
            ...
            const resizer = createElement("div", "absolute right-0 top-0 h-full w-2 translate-x-full cursor-col-resize rounded-r-full bg-transparent transition-colors hover:bg-sky-500/30");
        ...
        buildLeafElement(leaf) {
            ...
            const select = createElement("select", "min-w-[8rem] max-w-full shrink-0 rounded-md border border-slate-700 bg-slate-900/80 px-2 py-1 text-xs text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500/70");
        ...
        startPanelResize(group, index, event) {
            ...
            const onMove = (moveEvent) => {
                ...
                const nextPixelsA = clamp(initialPixelsA + delta, totalPixels * 0.15, totalPixels * 0.85);
        ...
        splitLeaf(leaf, orientation) {
            ...
            if (!parentInfo) {
                ...
                if (parent.orientation === orientation) {
                    ...
                    parent.sizes.splice(index + 1, 0, parent.sizes[index] ?? 1 / parent.children.length);
    ...
    class DataStore {
        ...
        createLandmassRecords() {
            ...
            this.game.forEachTile((ref) => {
                ...
                const anchorSummary = this.describeTile(component.anchorRef) ??
                    (this.game
                        ? {
